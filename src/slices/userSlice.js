...
const initialState = {
    ...
    export const fetchUsers = createAsyncThunk("userState/fetchUsers", async (last_created_at=null) => {
        const response = await fetch(`${getIpTenant()}users/${last_created_at?`last/${last_created_at}/`:''}`,
            {
    ...
    export const searchUsers = createAsyncThunk("userState/searchUsers", async (query) => {
        const response = await fetch(`${getIpTenant()}users/search/${query}/`, 
            { headers: { "Content-Type": "application/json", Accept: "application/json" }});
        return response.json();
    })

 export const deleteUser = createAsyncThunk("userState/deleteUser", async (id) => {
     const response = await fetch(`${getIpTenant()}users/${id}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' } });
     return response.json();
 });

 export const registerUser = createAsyncThunk("userState/registerUser", async (formData) => {
     
     const response = await fetch(`${getIpTenant()}users/`, { method: 'POST', body: formData });
     return response.json();
 });

export const updateUser = createAsyncThunk("userState/updateUser", async (formData) => {

    const response = await fetch(`${getIpTenant()}users/${formData.get("user[id]")}`,
          {
              method: 'PUT',
              body: formData,
          });
