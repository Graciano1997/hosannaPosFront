  ...
  setTimeout(()=>{
    ...
    return(
       <div className={`bg-black opacity-90 w-100 w-100  md:w-[400px]
        ...
        p-5 fixed bottom-[10px] right-[10px]
        rounded-[16px] flex-wrap`}>
        <XMarkIcon onClick={()=>{dispatch(closeToast())}} className={`w-6 h-6 ${object.success?'text-green-300':''} ${object.error?'text-white':''} ${object.warning?'text-yellow-300':''}  absolute right-2 top-[5px] cursor-pointer `}/>
        <p>{object.message}</p>
      </div>
    );
}
